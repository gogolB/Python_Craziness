# Helper functions for SMO Algorithm
from numpy import *
    ...
    while (iter < maxIter):
        ...
        for i in range(m):
            ...
            if ((labelMat[i] * Ei < -toler) and (alphas[i] < C)) or ((labelMat[i] * Ei > toler) and (alphas[i] > 0)):
                # Only Enters if optimization possible
                j = selectJrand(i, m);
...
# Calculate the kernel or transform data to a higher dimensional space.
...
# NOT A CLASS. An optimized data structure to carry around values.
class optStruct:
